...
#Hinzufügen der neuen Spalte Region mit bedingten Werten
df <- df %>%
...
col_order <- c(names(df)[1:which(names(df) == "georegion_type")], "Region", names(df)[(which(names(df) == "georegion_type")+1):(ncol(df)-1)])
df <- df[, col_order]
